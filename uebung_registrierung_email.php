...
if(isset($_POST["absenden"]))//erst wenn Absenden gedrückt wird
{
	...
	if($ok===true)
	{
        ...
        $userToken=md5(time());
        // Hash-Funktion von time
        $userRole=0;        
        //am Anfang hat er noch USreRolle =
            
       //Abstand nach user wichtig außer es ist in nächster ZEile 
        $sql="INSERT INTO user 
        (userName,userEmail,userPassword,userToken,userRole)
        VALUES
        (:name, :email, :password, :token, :role)";//: platzhalter, bei $userName im Formular eingegebenes wird in Kategorie userName etc in DB eingefügt
        ...
        $abfrage->bindParam(":token",$userToken);
        $abfrage->bindParam(":role",$userRole);
        $abfrage->execute();
        ...
        $empfaenger = $email;
        $betreff = "Verifizierung $userName";
        
        $nachricht = "Hallo\n";
        $nachricht .= "hier dein Link ";
        $nachricht .= "<a href='https://meinedomain.com/verifizieren.php?token=$userToken'>bitte klicken</a>";
